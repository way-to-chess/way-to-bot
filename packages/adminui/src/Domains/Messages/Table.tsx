...
const UserIds = () => {
    ...
    return type === "PICK" ?
        <Form.Item name={"userIds"} rules={[requiredRule]} valuePropName={"targetKeys"}>
            <UserTransfer/>
        </Form.Item> : null
}
...
const CreateForm = () => {
    const [sendCustomMessage, {isLoading}] = TGApi.useSendCustomMessageMutation()
    ...
    return <Form layout={"vertical"}
                 requiredMark={"optional"} onFinish={onFinish} disabled={isLoading}>

        <Form.Item name={"type"} rules={[requiredRule]} initialValue={"ALL"}>
            <Radio.Group
                options={[
                    {value: "ALL", label: "Для всех"},
                    {value: "PICK", label: 'Выбрать из списка', disabled: true},
                ]}
            />
        </Form.Item>
...
const MessagesTable = () => {
    ...
    return <>
        <Drawer title={"Отправить сообщение"} open={open} onClose={onClose} width={720}>
            <CreateForm/>
        </Drawer>
