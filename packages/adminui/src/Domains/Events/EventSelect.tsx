...
import {ESortDirection} from "@way-to-bot/shared/api/enums/index";
...
const EventOption: FC<Omit<AdminDTOEventGetMany, "countParticipants">> = (
    ...
    return <Flex align={"center"} gap={8}>
        <Avatar src={getPreviewSrc(preview?.previewUrl)} size={"large"} style={{flexShrink: 0}}/>
        <Flex vertical>
...
const optionRender: SelectProps["optionRender"] = ({data}) => <EventOption {...data as AdminDTOEventGetMany} />
...
const EventSelect: FC<TSelectProps> = (props) => {
    const {data: response, isFetching} = eventApi.useGetAllEventsQuery(queryOptions)
