...
const filterOption: TransferProps<AdminDTOUserGetMany>["filterOption"] = (
    inputValue,
    {firstName, lastName},
) =>
    String(getUserFullName(firstName, lastName))
        .toLowerCase()
...
const render: TransferProps<AdminDTOUserGetMany>["render"] = (user) => (
    <UserOption {...user} />
);

const UserTransfer: FC<TTransferProps> = (props) => {
    const {data: users, isFetching: usersIsFetching} =
        userApi.useGetAllUsersQuery(queryOptions);
    const [getEvent, {isFetching: eventsIsFetching}] =
        eventApi.useLazyGetEventByIdQuery();
    ...
    const onSelectChange: TransferProps<AdminDTOUserGetMany>["onSelectChange"] = (
        sourceSelectedKeys,
    ) => {
    ...
    const renderFooter: TransferProps<AdminDTOUserGetMany>["footer"] = (
        props,
        info,
    ) =>
        info?.direction === "left" ? (
