...
const optionsGoToEvent = (eventId: number, eventType: EEventType): TelegramBot.SendMessageOptions => {
    return {
        parse_mode: "HTML",
        reply_markup: {
            inline_keyboard: [
                [
                    {
                        ...
                        web_app: {url: `${WEB_URL}/${eventType}/events/${eventId}`},
                    },
                ],
            ],
        },
...
🎯 <b>Успейте записаться!</b>`;
    ...
    return {
        message,
        options: optionsGoToEvent(event.id, event.type),
    };
};
...
${pr.message?.trim() ? `\n💬 <b>Сообщение:</b>\n«${pr.message.trimEnd().trimStart()}»` : ""}`
    ...
    return {
        message,
        options: optionsGoToEvent(pr.event.id, pr.event.type),
    };
};
